...
describe('GET /todos/:id', () => {
    it('should return a todo back', (done) => {
        request(app)
    ...
    it('should not return a todo back created by another user', (done) => {
        request(app)
          .get(`/todos/${someTodos[1]._id.toHexString()}`)
          .set('x-auth', someUsers[0].tokens[0].token)
          .expect(404)
          .end(done);
    })
...
describe('DELETE /todos/:id', () => {
    ...
    it('should return 404 for no todo object ids', (done) => {
        ...
        request(app)
          .delete(`/todos/${hexId}`)
          .set('x-auth', someUsers[1].tokens[0].token)
          .expect(401)
          .end(done)
    })
})
...
describe('PATCH /todos/:id', () => {
    ...
    it('should not return and patch todo created by other user', (done) => {
        var id = someTodos[0]._id.toHexString();
        var text = 'new text for test';
    ...
    it('should remove auth token on logout', (done) => {
        request(app)
          ...
          .end((err, res) => {
              ...
              User.findById(someUsers[0]._id).then((user) => {
